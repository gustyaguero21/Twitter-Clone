...
func (pr *PostRepository) GetFollowingUsernames(username string) ([]string, error) {
	rows, err := pr.db.Query(config.ShowFollowersQuery, username)
	if err != nil {
		return nil, fmt.Errorf("error fetching following usernames: %v", err)
	}
	defer rows.Close()
